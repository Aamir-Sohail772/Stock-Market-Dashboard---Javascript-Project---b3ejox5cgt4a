//https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=AMZN&interval=5min&apikey=TTCY25MVPCB3LIJV
...
function createStockBox(stockBoxId, data, searchVal, timeFrame, timeFramesKey){
      ...
      let stockRateOpen = Object.values(data[timeFramesKey])[0]['1. open'];
        ...
        let stockCloseBtn = document.createElement("button");
          ...
          stockCloseBtn.setAttribute("id","closeIconeBtnId"+stockBoxId);
          
          stockCloseBtn.innerHTML = `<i class="fa fa-times" aria-hidden="true" style="pointer-events: none"></i>`;
          stockCloseBtn.addEventListener("click",function (evt){
            
                 let rmvDivId = evt.target.id.slice(15);
...
/*const stockBtn = {
      ...
      setManyAttributs(stockBox, stockBtn);
function setManyAttributs(elem, arrayAttributes) {
     Object.keys(arrayAttributes).forEach(attribute => {
      elem.setAttribute(attribute, arrayAttributes[attribute]);
       }); 
    } */
    ...
    /* <p id="stock1" class="BtnWatchList"> <span class="symbolTxt">AMZN</span> <span class="rateWatchListBox">123.45</span><button class="stockBtnArea">INTRADAY</button><button class="closeIconeBtn"><i class="fa fa-times" aria-hidden="true"></i></button></p>
    <p id="stock2" class="BtnWatchList"> <span class="symbolTxt">MSFT</span> <span class="rateWatchListBox">67.89</span><button class="stockBtnArea">DAILY</button><button class="closeIconeBtn" ><i class="fa fa-times" aria-hidden="true"></i></button></p>
    <p id="stock3" class="BtnWatchList"> <span class="symbolTxt">IBM</span> <span class="rateWatchListBox">56.34</span><button class="stockBtnArea">MONTHLY</button><button class="closeIconeBtn"><i class="fa fa-times" aria-hidden="true"></i></button></p>
*/
